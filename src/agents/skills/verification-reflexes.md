...
**Companion file: `verification-discipline`.** This file governs what you are allowed to claim. That one governs whether the check behind the claim measures anything: prove a test can fail before you trust it, run it rather than read it, cross the boundary the real request crosses, and never accept a PASS that asserts nothing. The gates here catch a claim with no check behind it. Nothing catches a check that passes while measuring the wrong thing, which is why that file exists. Read it whenever you are the one writing or running the check.
